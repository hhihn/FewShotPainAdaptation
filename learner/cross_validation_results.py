...
class CrossValidationResultRecorder:
    ...
    def record_heldout_size_result(
        ...
        *,
        ...
        """
        by_size = self.cv_results["heldout_eval_by_task_size"]
        if size_key not in by_size:
            k_shot, q_query = (int(part[1:]) for part in size_key.split("_"))
            by_size[size_key] = self._build_size_bucket(k_shot, q_query)
            self.cv_results["heldout_eval_task_sizes"].append(
